...
When an update lands in the background, the restart dialog depends on who can still show one. A new release tag re-patches an app that is already running the mod, so the app itself shows the same restart dialog as a manual check. A Codex update instead relaunches the app without the mod, so no mod code is loaded that could offer the restart; there the agent run that just re-patched the ASAR shows a system dialog offering to restart Codex, and declining it simply leaves the mod to activate on the next launch. A check that cannot reach the newest release, for example because the repository has diverged, reports a failure instead of pretending to be up to date.
